...
const Dashboard = () => {
  ...
  const fetchInviteCodes = async () => {
    ...
    if (codes) {
      ...
      if (usedByIds.length > 0) {
        ...
        profiles?.forEach(p => profileMap.set(p.user_id, p.display_name || "Без імені"));
      }
  ...
  const fetchGenerations = async () => {
    ...
    if (gens) {
      ...
      setGenerations(gens.map(g => ({
        ...g,
        user_name: profileMap.get(g.user_id || "") || "Невідомо"
      })));
    }
  };
  ...
  const handleGenerateCode = async () => {
    ...
    if (error) {
      ...
      toast({ title: "Код створено", description: newCode });
      fetchInviteCodes();
    }
    setGenerating(false);
  };
  ...
  const handleDeactivateCode = async (id: string) => {
    ...
    if (error) {
      toast({ title: "Помилка", description: "Не вдалося деактивувати код", variant: "destructive" });
    } else {
      fetchInviteCodes();
    }
  };
  ...
  const handleChangeRole = async () => {
    ...
    if (error) {
      ...
      toast({ title: "Збережено", description: "Роль змінено" });
  ...
  const handleApproveMember = async (memberId: string) => {
    ...
    if (error) {
      ...
      toast({ title: "Схвалено", description: "Члена команди схвалено" });
      fetchTeamMembers();
    }
  };
  ...
  const handleRejectMember = async (memberId: string) => {
    ...
    if (error) {
      toast({ title: "Помилка", description: "Не вдалося відхилити", variant: "destructive" });
    } else {
      toast({ title: "Відхилено", description: "Запит відхилено" });
      fetchTeamMembers();
    }
  };
      ...
      <div className="p-4 space-y-4 max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">Пульт власника</h1>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Членів команди</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Генерацій</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Активних кодів</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-xs">
                  Створюйте коди для запрошення нових членів команди
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div>
                  <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
                    <Badge variant="default" className="bg-green-500">Активні ({activeCodes.length})</Badge>
                  </h3>
                  {activeCodes.length === 0 ? (
                    <p className="text-sm text-muted-foreground py-4 text-center">Немає активних кодів</p>
                  ) : (
                ...
                {usedCodes.length > 0 && (
                  <div>
                    <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
                      <Badge variant="secondary">Використані ({usedCodes.length})</Badge>
                    </h3>
                    <div className="space-y-2">
                      {usedCodes.slice(0, 10).map(code => (
                        <div key={code.id} className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
                          ...
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            <span>•</span>
                            <span>{code.used_at ? new Date(code.used_at).toLocaleDateString("uk-UA") : ""}</span>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {pendingMembers.length > 0 && (
                  <div>
                    <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
                      <Badge variant="destructive">Очікують схвалення ({pendingMembers.length})</Badge>
                    </h3>
                    <div className="space-y-2">
                ...
                <div>
                  <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
                    <Badge variant="default">Активні члени ({approvedMembers.length})</Badge>
                  </h3>
                  <div className="space-y-2">
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm flex items-center gap-2">
                  ...
                  Історія генерацій команди
                </CardTitle>
                <CardDescription className="text-xs">
                  Всього: {completedGens.length} генерацій • Продажі: ${totalSales.toFixed(2)} • Витрати: ${totalCosts.toFixed(2)}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {generations.length === 0 ? (
                  <p className="text-sm text-muted-foreground py-8 text-center">Немає генерацій</p>
                ) : (
                    ...
                    {generations.map(gen => (
                      <div key={gen.id} className="flex items-center justify-between p-3 rounded-lg border bg-card">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-sm truncate">{gen.site_name || "Без назви"}</span>
                            <Badge 
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-4 rounded-lg border bg-blue-500/10 border-blue-500/30">
                    <p className="text-sm text-muted-foreground">Кредитний ліміт</p>
                  ...
                  <div className="p-4 rounded-lg border bg-purple-500/10 border-purple-500/30">
                    <p className="text-sm text-muted-foreground">Доступно для витрат</p>
                ...
                <div className="p-4 rounded-lg border">
                  <h3 className="font-medium mb-2">Статистика генерацій</h3>
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="text-2xl font-bold">{completedGens.length}</p>
                      <p className="text-xs text-muted-foreground">Завершених</p>
                    </div>
                    <div>
                      <p className="text-2xl font-bold text-green-600">${totalSales.toFixed(2)}</p>
                      <p className="text-xs text-muted-foreground">Сума продажів</p>
                    </div>
                    <div>
                      <p className="text-2xl font-bold text-amber-600">${totalCosts.toFixed(2)}</p>
                      <p className="text-xs text-muted-foreground">Витрати AI</p>
                    </div>
                  </div>
                </div>
        ...
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-sm flex items-center gap-2">
              <UserCog className="h-4 w-4" />
              Змінити роль: {selectedMember?.display_name}
            </DialogTitle>
          </DialogHeader>
