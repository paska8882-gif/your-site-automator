  ...
  try {
    ...
    if (status === "done" || status === "completed") {
      ...
      if (zipFile) {
        ...
        // --- Auto-retry DISABLED: just log warning if ZIP is small ---
        if (zipSizeBytes < MIN_ZIP_SIZE_BYTES) {
          console.log(`⚠️ ZIP is small (${(zipSizeBytes / 1024).toFixed(1)}KB < 95KB), but auto-retry is disabled. Accepting result as-is.`);
        }
