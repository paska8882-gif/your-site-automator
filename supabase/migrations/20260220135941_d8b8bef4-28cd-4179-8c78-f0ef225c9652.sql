...
-- Create a database function that replaces the cleanup-stale-generations Edge Function
...
BEGIN
  ...
  -- Process stale generations (>1 hour)
  FOR v_stale_row IN
  ...
  LOOP
    ...
    IF v_stale_row.user_id IS NOT NULL THEN
      ...
      IF v_existing_appeal_id IS NULL THEN
        ...
        VALUES (
          ...
          'pending',
          v_refund,
          '⏱️ Auto-timeout 1h. Suggested refund: $' || v_refund::text
        );
    ...
    SET status = 'failed',
        error_message = 'Перевищено час очікування (1 год). Апеляцію створено автоматично.'
